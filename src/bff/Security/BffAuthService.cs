...
public sealed class BffAuthOptions
{
    ...
    public bool EnableDefaultDevelopmentMembership { get; init; } = true;
    public bool EnablePublicOnboarding { get; init; }
    ...
    IWebHostEnvironment environment) : IBffAuthService
{
    public async Task<BffSignInResult> SignInWithPasswordAsync(BffLoginRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var memberships = currentOptions.DevelopmentMemberships.Count == 0 && currentOptions.EnableDefaultDevelopmentMembership
                ? [CreateTenantResponse(Guid.Parse("11111111-1111-1111-1111-111111111111"), "Demo Clinic", "admin", null)]
                : currentOptions.DevelopmentMemberships.Select(WithPermissions).ToArray();
        ...
        CancellationToken cancellationToken)
    {
        ...
        return memberships
            .Select(membership => CreateTenantResponse(
                membership.TenantId,
    ...
    private static BffTenantResponse WithPermissions(BffTenantResponse membership)
        => CreateTenantResponse(membership.TenantId, membership.TenantName, membership.Role, membership.ProfessionalId);

    private static BffTenantResponse CreateTenantResponse(Guid tenantId, string tenantName, string role, Guid? professionalId)
        => new(
            tenantId,
            tenantName,
            role,
